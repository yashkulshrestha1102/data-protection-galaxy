...
export async function POST(request: Request) {
  try {
    ...
    const { name, email, company, designation, score, riskLevel, categoryScores } = body;

    console.log('📊 Score Report Request:');
    ...
    try {
      ...
      // Agar API key nahi hai toh error throw karo
          ...
          <body>
            ...
            <div style="background: #ecfdf5; padding: 15px; border-radius: 8px; border: 1px solid #10b981; margin: 20px 0;">
              ...
              <p style="color:#6b7280; margin:10px 0 0 0;">
                ...
                Visit our <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://data-protection-galaxy.vercel.app'}/galaxy">Galaxy</a> for curated resources.
              </p>
            </div>
            
            <div style="text-align:center; margin: 30px 0;">
              <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://data-protection-galaxy.vercel.app'}/certificate-course" style="display:inline-block; background: linear-gradient(to right, #7c3aed, #6d28d9); color: white; padding: 12px 30px; border-radius: 8px; text-decoration: none;">
    ...
    return NextResponse.json({
      success: true,
      emailSent: emailSent,
      message: emailSent ? 'Report sent successfully!' : 'Report generated but email could not be sent. Please check Resend configuration.',
    });
  } catch (error) {
    console.error('❌ Error in score-report API:', error);
    return NextResponse.json(
